...
case class IPv4Network(ipNetwork: String) extends IPv4Traits {
  ...
  private val (addr: String, cidr: Int) = {
    ...
    else throw new Exception("Bad IPv4 Network.")
  }
  ...
  val (addrLStart: Long, addrLEnd: Long) = {
    ...
    (if (IP2.isDefined) addrL else 0xFFFFFFFF << (32 - cidr) & addrL,
      if (IP2.isDefined) {
        val temp = IPv4ToLong(IP2.getOrElse(throw new Exception("Bad IPv4 Network Range.")))
        require(temp > addrL, "Bad IPv4 Network Range.")
        temp
      } else addrL | ((1 << (32 - cidr)) - 1))
  }
  ...
  def contains(ip: Any): Boolean = ip match {
    case v4: IPv4 => v4.addrL >= addrLStart && v4.addrL <= addrLEnd
    case str: String =>
      try {
        val ipaddr = IPv4(str)
        ipaddr.addrL >= addrLStart && ipaddr.addrL <= addrLEnd
      }
      catch {
